...
async function checkout() {
    ...
    if (isScheduling) {
        ...
        if (!scheduledTime) {
            alert("Por favor, selecione um horário para a entrega ou retirada.");
            return;
        }
    }
    ...
    try {
        ...
        if (isScheduling) {
            ...
            const dataFormatada = amanha.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' });

            message += `*‼️ PEDIDO AGENDADO ‼️*\n`;
            message += `*RETIRADA/ENTREGA PARA: ${dataFormatada}*\n`;
            message += `*HORÁRIO SOLICITADO: ${scheduledTime}*\n\n`;
        }
        ...
        // Esta verificação foi movida para depois da criação da URL
        if (isScheduling) {
            // Não removemos mais a flag aqui para que a página não volte ao normal
            // A remoção agora acontece no início do carregamento da página
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ===== PONTO DA CORREÇÃO =====
    // Se detectamos a flag de agendamento, nós a removemos imediatamente.
    // Isso garante que ela só seja válida para este carregamento de página.
    // Se o usuário recarregar a página manualmente (F5), a flag não existirá mais.
    if (isScheduling) {
        sessionStorage.removeItem('isSchedulingOrder');
    }
    // =============================

    // A verificação de segurança para remover a flag se a loja abrir continua útil.
    ...
    if (isScheduling) {
        ...
        if (paymentMethodSelect) {
            ...
            for (const option of paymentMethodSelect.options) {
                if (option.value !== 'Pix' && option.value !== '') {
