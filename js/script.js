...
// ===============================================
const DADOS_LOJA = {
    // Horário com minutos (ex: 12.5 = 12:30, 17.5 = 17:30)
    horarioAbertura: 12.5,
    horarioFechamento: 17.5,
    // A loja agora funciona todos os dias
    diasFuncionamento: [0, 1, 2, 3, 4, 5, 6] // 0 a 6 = Domingo a Sábado
};
...
function lojaEstaAberta() {
    ...
    // Ajuste para o fuso horário de Brasília (GMT-3) - importante para servidores em outros locais
...
// Busca os produtos no Contentful
...
// Renderiza os produtos na tela, considerando se a loja está aberta ou se há estoque
    ...
    categoryOrder.forEach(categoria => {
        if (productsByCategory[categoria]) {
            ...
            productsByCategory[categoria].forEach(product => {
                ...
                let buttonHTML = '';
                if (!lojaAberta) {
                    buttonHTML = `<button class="add-button-esgotado" disabled>Fechado</button>`;
                } else if (product.estoque > 0) {
                    buttonHTML = `<button class="add-button" onclick="addToCart('${product.id}')">+</button>`;
                } else {
                    buttonHTML = `<button class="add-button-esgotado" disabled>Esgotado</button>`;
                }
                productElement.className = `product-item ${!lojaAberta || product.estoque <= 0 ? 'esgotado' : ''}`;
                productElement.dataset.productId = product.id;
                productElement.innerHTML = `<div class="product-info"><h4 class="product-name">${product.name}</h4><p class="product-description">${product.description}</p><p class="product-price">R$ ${product.price.toFixed(2).replace('.', ',')}</p></div><div class="product-image-container"><img src="${product.image}" alt="${product.name}" class="product-image">${buttonHTML}</div>`;
...
// Atualiza os valores totais no carrinho (subtotal, taxa, total)
